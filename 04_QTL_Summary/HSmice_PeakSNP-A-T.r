...
chr_rm = c(23); phe_rm = c()

AD_dir = "/SAN/mottlab/heterosis/4_HSmice/3NonAdd_QTL/1_A+D" 
T_dir = "/SAN/mottlab/heterosis/4_HSmice/3NonAdd_QTL/2_T-statistics" 
folder_name = "4_HSmice"; file_name = "HSmice";
CI_Pvalue_decrease = 2; CI_Pvalue_r2 = 0.8; Distance = 2000000 # Distance allow QTL and QTL(or eQTL) are shared one #

##################################################################################################
### Part1: PeakSNP-A_T.r (Based on A P-value to choose the QTLs from A+D Model)
##################################################################################################
...
try({
	...
	if(!file.exists("PeakSNP_A.txt")){
		...
		try({
			...
			if(max(tmp_AD[,"X.logP.AddCode."]) > thrsuggest){
				tmp_result_all1 = c()
				tmp_AD_Sig = subset(tmp_AD,tmp_AD[,"X.logP.AddCode."]  > thrsuggest)
				for(j in 1:length(unique(tmp_AD_Sig[,"chr"]))){
					...
					tmp_AD_Sig_max = subset(tmp_AD_Sig_chr,tmp_AD_Sig_chr[,"X.logP.AddCode."] == max(tmp_AD_Sig_chr[,"X.logP.AddCode."]))[1,]
					...
					tmp_AD_chr = subset(tmp_AD,tmp_AD[,"chr"] == unique(tmp_AD_Sig[,"chr"])[j] & tmp_AD[,"X.logP.AddCode."] >= (tmp_AD_Sig_max[,"X.logP.AddCode."]-CI_Pvalue_decrease))
		...
		write.table(result_all1,file="PeakSNP_A.txt",row.names=F,col.names=T,quote=F)

	}

	# STEP3: Filter overlap QTLs in two methods and SNPs with biggest P-value that are both >thrsuggest #

	result_all1 = read.table(paste0(AD_dir,"/PeakSNP_A.txt"),header=T)
	...
	for(i in 1:nrow(result_all)){
		...
		if(tmp_qtl1[3] == tmp_qtl2[3]){ # Case1: If tmp_qtl1 and tmp_qtl2 share the common peak SNP #
			...
			region_AD_common_Sig = region_AD_common[region_AD_common[,"X.logP.AddCode."]>thrsuggest1,] # Get the common region & significant SNPs for tmp_qtl1 #
			...
			if(nrow(region_AD_common_Sig) != 0 && nrow(region_T_common_Sig) != 0){
				...
				if(length(common_SNP_Sig) > 0){
					...
					tmp_qtl =c(tmp_qtl1[1], as.character(region_AD_common_Key[region_AD_common_Key[,"X.logP.AddCode."]==max(region_AD_common_Key[,"X.logP.AddCode."]),1:2][1,]))
	...
	write.table(result_all,file="PeakSNP-A_T-Common1.txt",row.names=F,col.names=T,quote=F) # Contains all the overlap QTL-eQTL pairs and the 3rd col is the "Both-Sig SNP" #
	write.table(result_detail,file="PeakSNP-A_T-Common2.txt",row.names=F,col.names=T,quote=F) # Just contains the "Both-Sig SNPs" from overlap QTL-eQTL pairs #
