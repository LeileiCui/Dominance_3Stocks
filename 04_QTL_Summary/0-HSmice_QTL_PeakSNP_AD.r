...
out_dir = "/SAN/mottlab/heterosis/4_HSmice/3NonAdd_QTL/1_A+D/SNP_Peak"
rawdat_dir = "/SAN/mottlab/heterosis/4_HSmice/1data/HSmice"
pvalue_dir = "/SAN/mottlab/heterosis/4_HSmice/3NonAdd_QTL/1_A+D/2_Pvalue"

CI_Pvalue_decrease = 2; CI_Pvalue_r2 = 0.8; pvalue_choose = 4
pvalue_filter = c("X.logP.AddDomCode_Add.","X.logP.AddCode.","X.logP.DomCode.","X.logP.AddDomCode_Null.","X.logP.IndiCode.")[pvalue_choose]
pvalue_outname = c("PeakSNP_AvsAD_thrsuggest.txt", "PeakSNP_A_thrsuggest.txt", "PeakSNP_D_thrsuggest.txt", "PeakSNP_AD_thrsuggest.txt", "PeakSNP_Indi_thrsuggest.txt")[pvalue_choose]
...
try({
	...
	if(max(tmp_AD[, pvalue_filter]) > thrsuggest){
		...
		colnames(tmp_result_all1) = c("trait","chr","pos","logP_AD_A","logP_A","logP_D","logP_AD","logP_Indi","beta_Add","beta_OverDom","OD/A","MAF","CI_r2","CI_logP","thrgenome","thrsuggest")
