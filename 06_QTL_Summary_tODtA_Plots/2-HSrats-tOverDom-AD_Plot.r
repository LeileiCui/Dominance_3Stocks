...
setwd("/SAN/mottlab/heterosis/3_HSrats/3NonAdd_QTL/1_A+D_0.999999r2/2_Pvalue"); chr_rm = c(23)
file_all  = dir(); SizeMatrix = length(file_all)
color_sum = read.table("/SAN/mottlab/heterosis/3_HSrats/1data/HSrats.cols",header=T,sep="\t")

# png_title = "tODtA_AvsAD_Plot"; pchoose_col = "X.logP.AddDomCode_Add."; model_name = "AvsAD"; stock_name = "HS Rats"
# png_title = "tODtA_D_Plot"; pchoose_col = "X.logP.DomCode."; model_name = "D"; stock_name = "HS Rats"
# png_title = "tODtA_A_Plot"; pchoose_col = "X.logP.AddCode."; model_name = "A"; stock_name = "HS Rats"
png_title = "tODtA_AD_Plot"; pchoose_col = "X.logP.AddDomCode_Null."; model_name = "AD"; stock_name = "HS Rats"
...
try({
	...
	# text(x = plot_x[seq(1,length(plot_x),2)], y = axis_pos, srt = 45, adj = 1, labels = gsub(".resid","",SigTraits_Peak[,"trait"])[seq(1,length(plot_x),2)], xpd = TRUE, cex = 0.8)
	# text(x = plot_x[seq(2,length(plot_x),2)], y = axis_pos-1.6, srt = 45, adj = 1, labels = gsub(".resid","",SigTraits_Peak[,"trait"])[seq(2,length(plot_x),2)], xpd = TRUE, cex = 0.8)
	ratio_xlab = c(3.5, 15.5, 25.5, 36.0, 68.5, 111.5, 130.0, 141.5, 155.0, 162.0, 170.0, 180.0)
	text(x = ratio_xlab, y = axis_pos+0.5, adj = 0.5, labels = paste0(ratio_plot[,3], " (", signif(as.numeric(ratio_plot[,4])*100,3), "%)"), col = ratio_plot[,5], xpd = TRUE, cex = 1.5)
