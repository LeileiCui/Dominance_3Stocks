...
setwd("/SAN/mottlab/heterosis/4_HSmice/3NonAdd_QTL/1_A+D/2_Pvalue"); chr_rm = c(23)
file_all  = dir(); SizeMatrix = length(file_all)
color_sum = read.table("/SAN/mottlab/heterosis/4_HSmice/1data/HSmice.cols",header=T,sep="\t")

# png_title = "tODtA_AvsAD_Plot"; pchoose_col = "X.logP.AddDomCode_Add."; model_name = "AvsAD"; stock_name = "HS Mice"
# png_title = "tODtA_D_Plot"; pchoose_col = "X.logP.DomCode."; model_name = "D"; stock_name = "HS Mice"
# png_title = "tODtA_A_Plot"; pchoose_col = "X.logP.AddCode."; model_name = "A"; stock_name = "HS Mice"
png_title = "tODtA_AD_Plot"; pchoose_col = "X.logP.AddDomCode_Null."; model_name = "AD"; stock_name = "HS Mice"
...
try({
	...
	text(x = plot_x, y = axis_pos, srt = 45, adj = 1, labels = gsub(".resid","",SigTraits_Peak[,"trait"]), xpd = TRUE, cex = 0.85)
	...
	# text(x = plot_x[seq(2,length(plot_x),2)], y = axis_pos-1.5, srt = 90, adj = 1, labels = gsub(".resid","",SigTraits_Peak[,"trait"])[seq(2,length(plot_x),2)], xpd = TRUE, cex = 0.8)
	ratio_xlab = c(0, 6, 12, 23, 39.5, 51, 58.0, 63.5, 71.5, 80.0, 90.0, 98.0, 108.5)
