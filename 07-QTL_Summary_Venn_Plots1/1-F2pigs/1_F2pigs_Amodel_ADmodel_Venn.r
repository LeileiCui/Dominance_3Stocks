...
setwd("C:\\Users\\Administrator\\Desktop\\1-F2pigs")
...
QTL_data_AD = read.table("F2pigs_QTL_AD.txt",header=T)
QTL_data_A = read.table("F2pigs_QTL_A.txt",header=T)
ploteQTL(dat_AD=QTL_data_AD, dat_A=QTL_data_A, plotname="F2pigs")

###############################################################################################################################

rm(list=ls());options(stringsAsFactors=FALSE)
setwd("/Users/leileicui/Desktop/1_项目资料/2_NonAdditive/结果汇总/Heterosis_Results_2【初步整理结果】/3-3-显著QTL检测优势对比-1/1-F2pigs")
...
QTL_data_AD = read.table("F2pigs_QTL_AD.txt",header=T)
QTL_data_A = read.table("F2pigs_QTL_A.txt",header=T)
ploteQTL(dat_AD=QTL_data_AD, dat_A=QTL_data_A, plotname="F2pigs1")