...
outdir = "/SAN/mottlab/heterosis/4_HSmice/3NonAdd_eQTL_Res/1_A+D_hippo"

covariates_types = c("n", "n", "f"); names(covariates_types) = c("sex", "BW", "batch")
PheList = c("id", "sex", "BW", "batch", "HP.express.X9790357_5448.S", "HP.express.X9845300_1012.S", "HP.express.X9845300_1171.S")

# [3] Use three functions one by one #
ADDO_AddDom1_QC(indir=indir, outdir=outdir, Input_name="HSmice_hippo", Input_type="PLINK", Kinship_type="GCTA_ad", PheList_Choose=F, PheList=PheList, Phe_ResDone = F, Phe_NormDone = F, Normal_method = "QUANTILE", covariates_sum=3, covariates_types=covariates_types, Phe_IndMinimum = 150, Phe_Extreme = 5, GT_maf = 0.05, GT_missing = 0.1, num_nodes=15)

ADDO_AddDom2_Pvalue(indir=indir, outdir=outdir, Input_name="HSmice_hippo", Kinship_type="GCTA_ad", VarComponent_Method="GCTA_ad", PheList_Choose=F, PheList=PheList, covariates_sum=3, Phe_IndMinimum=150, GT_IndMinimum=10, matrix_acceleration=F, num_nodes=15)
