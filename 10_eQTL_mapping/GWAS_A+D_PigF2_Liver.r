...
source("../3_GWAS_AddDom_Plot_All_AD.r"); # Plotting Traits >thrsuggest & based on NvsAD-Pvalues # 
...
outdir = "/SAN/mottlab/heterosis/1_F2pigs/3NonAdd_eQTL/1_A+D_L"

covariates_types = c("f","f","n","n","n","n","n","n","n","n","n","n")
names(covariates_types) = c("sex","batch","PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10")
PheList = c("id","sex","batch","PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10",gsub("EMMA_Plot_","",gsub(".png","",dir("../Sig-L/"))))

# [3] Use three functions one by one #
#GWAS_AddDom_QC(indir=indir, outdir=outdir, Input_name="F2_DGE_L", Input_type="PLINK", Kinship_type="GCTA_ad", PheList_Choose=F, PheList=PheList, Phe_ResDone = F, Phe_NormDone = F, Normal_method = "QUANTILE", covariates_sum=12, Phe_IndMinimum = 200, Phe_Extreme = 5, GT_maf = 0.05, GT_missing = 0.1, num_nodes=15)

#GWAS_AddDom_Pvalue(indir=indir, outdir=outdir, Input_name="F2_DGE_L", Kinship_type="GCTA_ad", VarComponent_Method="GCTA_ad", PheList_Choose=F, PheList=PheList, Run_separated = F, Run_separated_length = 300000,covariates_sum=12, covariates_types=covariates_types, Phe_IndMinimum=200, GT_IndMinimum=10, num_nodes=15)
