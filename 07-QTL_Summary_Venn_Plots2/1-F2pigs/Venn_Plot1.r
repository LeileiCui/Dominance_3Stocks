...
Stock_name = c("F2pigs"); data_dir = c("/SAN/mottlab/heterosis/1_F2pigs"); Sig_level = "suggest"

###################################################################################################################
### Part1: Calculate the sum of significant SNPs in 7 models A/D/AD/AD_A/AD_D/Indi/T and their overlaps SNP num ###
###################################################################################################################

AD_dir = paste0(data_dir,"/3NonAdd_QTL/1_A+D/2_Pvalue"); T_dir = paste0(data_dir,"/3NonAdd_QTL/2_T-statistics/2_Pvalue")
...
Map_Info = read.table(paste0(data_dir,"/3NonAdd_QTL/1_A+D/1_PheGen/0_Data/Data_clean.map"),header=F)
...
Stock_name = c("F2pigs"); Sig_level = "suggest"
...
### Remove the Sig-SNPs from X chromosome and bad phenotypes ###
result_A_all = subset(result_A_all,!(result_A_all[,"trait"] %in% phe_rm) & !(result_A_all[,"chr"] %in% chr_rm))
result_AD_all = subset(result_AD_all,!(result_AD_all[,"trait"] %in% phe_rm) & !(result_AD_all[,"chr"] %in% chr_rm))
result_D_all = subset(result_D_all,!(result_D_all[,"trait"] %in% phe_rm) & !(result_D_all[,"chr"] %in% chr_rm))
result_AD_A_all = subset(result_AD_A_all,!(result_AD_A_all[,"trait"] %in% phe_rm) & !(result_AD_A_all[,"chr"] %in% chr_rm))
result_T_all = subset(result_T_all,!(result_T_all[,"trait"] %in% phe_rm) & !(result_T_all[,"chr"] %in% chr_rm))

res_A_all = rownames(result_A_all); res_AD_all = rownames(result_AD_all); res_D_all = rownames(result_D_all); res_AD_A_all = rownames(result_AD_A_all); res_T_all = rownames(result_T_all)
res_all = sort(unique(c(res_A_all,res_AD_all,res_D_all,res_AD_A_all,res_T_all))); names(res_all) = 1:length(res_all)
res_A_allcode = names(res_all[res_all %in% res_A_all])
res_AD_allcode = names(res_all[res_all %in% res_AD_all])
res_D_allcode = names(res_all[res_all %in% res_D_all])
res_AD_A_allcode = names(res_all[res_all %in% res_AD_A_all])
...
try({
	...
	### Part3-1: Box Plot of -logP value ###
		...
		Pname1 = "X.logP.AddDomCode_Null."; Pname2 = "X.logP.AddDomCode_Add."
		col1 = "red1"; col2 = "red4"
	...
	### Part3-2: Box Plot of -logP value ###
		...
		Pname1 = "X.logP.AddDomCode_Null."; Pname2 = "MVN_logP_Minor"
		col1 = "blue1"; col2 = "blue4"
