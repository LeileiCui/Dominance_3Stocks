...
outdir = "/SAN/mottlab/heterosis/3_HSrats/5VC_gene/HSrats_amygdala"; setwd(outdir)
resname = "HSrats_amygdala"
...
# write.table(col_dat, file=paste0(resname, "_GeneVC_Colors.txt"), row.names=F, col.names=T, quote=F)
col_dat = read.table(paste0(resname, "_GeneVC_Colors.txt"), header=T); rownames(col_dat) = col_dat[,1]
