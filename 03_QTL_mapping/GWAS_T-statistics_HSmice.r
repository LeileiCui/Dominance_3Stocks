...
outdir = "/SAN/mottlab/heterosis/4_HSmice/3NonAdd_QTL/2_T-statistics"

covariates_types = c(); names(covariates_types) = c(); PheList = c()

# [3] Use three functions one by one #
GWAS_Tstatistics_QC(indir=indir, outdir=outdir, Input_name="HSmice", Input_type="PLINK", Kinship_type="GCTA_ad", PheList_Choose=F, PheList=PheList, Phe_ResDone = T, Phe_NormDone = F, Normal_method = "QUANTILE", covariates_sum=1, Phe_IndMinimum = 200, Phe_Extreme = 5, GT_maf = 0.05, GT_missing = 0.1, num_nodes=15)

GWAS_Tstatistics_Pvalue(indir=indir, outdir=outdir, Input_name="HSmice", Kinship_type="GCTA_ad", VarComponent_Method="GCTA_ad", PheList_Choose=F, PheList=PheList, Run_separated = F, Run_separated_length = 300000,covariates_sum=1, covariates_types=covariates_types, Phe_IndMinimum=200, GT_IndMinimum=10, num_nodes=15)

GWAS_Tstatistics_Plot(outdir=outdir, PheList_Choose=F, PheList=PheList, covariates_sum=1, RegionMan_chr_whole=F, RegionMan_chr_region = 15000000, num_nodes=15)
