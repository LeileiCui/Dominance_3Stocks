...
eQTL_filenames = c("HSrats_amygdala_eQTL_AD_g.txt", "HSrats_heart_eQTL_AD_g.txt")
...
for(name_sort in 1:length(eQTL_filenames)){
	...
	write.table(eQTL_results, file=paste0("Summary_", res_name, "_eQTL_enrichment_g.txt"), row.names=F, col.names=T, quote=F, sep="\t")
...
for(name_sort in 1:length(res_filenames)){
	...
	eQTL_results_tmp = read.table(paste0("Summary_", res_filenames[name_sort], "_eQTL_enrichment_g.txt"), header=T)
...
for(name_sort in 1:length(res_filenames)){
	...
	if(name_sort == 1){
		...
		png(paste0("Plot_eQTL_enrichment_g_newcol.png"), width=4300, height=4000, res=600, type="cairo")

		par(mar=c(4,4,2,4.5), mgp=c(2.2,0.8,0), cex.main=1.5)
		plot(data_x, data_y1, type="l", col=plot_cols[name_sort], xlab="Significant thresholds of eQTLs (HSrats_g)", ylab="-logP of Fisher test of eQTL trans-acting enrichment", lty=1, xlim=c(0,xlim_max), ylim=c(0,ylim_max1), lwd=lwd_used)
