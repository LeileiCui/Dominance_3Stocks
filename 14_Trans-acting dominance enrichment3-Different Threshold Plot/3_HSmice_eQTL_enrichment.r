...
setwd("/Users/leileicui/Desktop/1_项目资料/2_NonAdditive/结果汇总/Heterosis_Results_2【初步整理结果】/6-4-Dom Enrichment-Different Threshold Plot/3-HSmice")

eQTL_filenames = c("HSmice_hippocampus_eQTL_AD.txt", "HSmice_liver_eQTL_AD.txt", "HSmice_lung_eQTL_AD.txt")
res_filenames = c("HSmice_hippocampus", "HSmice_liver", "HSmice_lung")
...
for(name_sort in 1:length(eQTL_filenames)){
	...
	write.table(eQTL_results, file=paste0("Summary_", res_name, "_eQTL_enrichment.txt"), row.names=F, col.names=T, quote=F, sep="\t")
...
res_filenames = c("F2pigs_liver", "F2pigs_muscle", "HSrats_amygdala", "HSrats_heart", "HSmice_hippocampus", "HSmice_liver", "HSmice_lung")[5:7]
# plot_cols = c("black", "bisque4", "bisque")[1:3]
plot_cols = c("black", "orange", "olivedrab")[1:3]

ylim_max1_all = c(); ylim_max2_all = c()
for(name_sort in 1:length(res_filenames)){

	eQTL_results_tmp = read.table(paste0("Summary_", res_filenames[name_sort], "_eQTL_enrichment.txt"), header=T)
...
for(name_sort in 1:length(res_filenames)){
	...
	if(name_sort == 1){
		...
		png(paste0("Plot_eQTL_enrichment_newcol.png"), width=4300, height=4000, res=600, type="cairo")

		par(mar=c(4,4,2,4.5), mgp=c(2.2,0.8,0), cex.main=1.5)
		plot(data_x, data_y1, type="l", col=plot_cols[name_sort], xlab="Significant thresholds of eQTLs (HSmice)", ylab="-logP of Fisher test of eQTL trans-acting enrichment", lty=1, xlim=c(0,xlim_max), ylim=c(0,ylim_max1), lwd=lwd_used)
