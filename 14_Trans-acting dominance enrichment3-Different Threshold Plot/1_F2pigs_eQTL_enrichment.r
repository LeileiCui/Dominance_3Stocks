...
setwd("/Users/leileicui/Desktop/1_项目资料/2_NonAdditive/结果汇总/Heterosis_Results_2【初步整理结果】/6-4-Dom Enrichment-Different Threshold Plot/1-F2pigs")

eQTL_filenames = c("F2pigs_liver_eQTL_AD.txt", "F2pigs_muscle_eQTL_AD.txt")
res_filenames = c("F2pigs_liver", "F2pigs_muscle")
...
res_filenames = c("F2pigs_liver", "F2pigs_muscle", "HSrats_amygdala", "HSrats_heart", "HSmice_hippocampus", "HSmice_liver", "HSmice_lung")[1:2]
# plot_cols = c("black", "bisque4", "bisque")[1:2]
plot_cols = c("black", "orange", "olivedrab")[1:2]
...
for(name_sort in 1:length(res_filenames)){
	...
	if(name_sort == 1){
		...
		plot(data_x, data_y1, type="l", col=plot_cols[name_sort], xlab="Significant thresholds of eQTLs (F2pigs)", ylab="-logP of Fisher test of eQTL trans-acting enrichment", lty=1, xlim=c(0,xlim_max), ylim=c(0,ylim_max1), lwd=lwd_used)
